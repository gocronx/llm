...
def test_resume_required() -> bool:
    r = extract(
        ...
        _model,
        "提取简历信息。",
        "李四，30岁，前端工程师 ls@x.com，会 React、Vue",
        RESUME,
    )
    ok = (
        all(k in r for k in ["name", "age", "position", "email", "skills"])
        and isinstance(r["age"], int)
        and isinstance(r["skills"], list)
    )
...
def test_product_nested_number() -> bool:
    r = extract(
        ...
        _model,
        "提取产品信息。",
        "AirPods Pro 1899 元人民币，苹果，有货。",
        PRODUCT,
    )
    ok = isinstance(r["price"]["amount"], (int, float)) and r["price"]["currency"] in (
        "CNY",
        "USD",
        "EUR",
    )
...
def test_sentiment_enum() -> bool:
    r = extract(
        _client, _model, "对文本做情感分类。", "服务很差，再也不来了。", SENTIMENT
    )
...
def main() -> None:
    passed = sum(
        [test_resume_required(), test_product_nested_number(), test_sentiment_enum()]
    )
    print(f"\n{passed}/3 passed")
