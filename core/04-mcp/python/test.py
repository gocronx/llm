...
async def run() -> int:
    ...
    params = StdioServerParameters(
        ...
        args=[str(here / "server.py"), str(workspace)],
    )
    passed = 0
    async with AsyncExitStack() as stack:
        ...
        await session.call_tool(
            "write_file", {"path": "hello.txt", "content": "hi mcp"}
        )
        out = (
            (await session.call_tool("read_file", {"path": "hello.txt"}))
            .content[0]
            .text
        )
        ok = "hi mcp" in out
        ...
        listing = (
            (await session.call_tool("list_directory", {"path": "."})).content[0].text
        )
