...
def scenario_fallback() -> None:
    ...
    with FakeServer() as a, FakeServer() as b:
        a.script("/chat/completions", *[(500, None, "down")] * 8)  # primary 全挂
...
def scenario_circuit_breaker() -> None:
    ...
    with FakeServer() as a, FakeServer() as b:
        ...
        b.script(
            "/chat/completions", (200, None, "secondary"), (200, None, "secondary")
        )
        client = Resilient(
            [
                Endpoint(
                    _client(a.base_url),
                    "fake",
                    breaker=Breaker(threshold=5, cooldown_seconds=30),
                    label="primary",
                ),
                Endpoint(_client(b.base_url), "fake", label="secondary"),
            ],
