...
def main() -> None:
    passed = sum(
        [
            ...
            t(
                "regex 6 digits",
                regex("100000", r"^\d{6}$") and not regex("12abc", r"^\d{6}$"),
            ),
            t(
                "keywords AND + 同义词",
                keywords("使用 TLS 加密和证书", ["TLS|SSL", "证书|身份"])
                and not keywords("使用 TLS 加密", ["TLS|SSL", "证书|身份"]),
            ),
            ...
            t(
                "evaluate dispatch exact",
                evaluate("2", {"id": "x", "metric": "exact", "expected": "2"})[0],
            ),
            t(
                "evaluate dispatch regex",
