...
def main() -> None:
    ...
    print(f"   ↑ 越大数 gap 越大 (exp 主导), 越小数 gap 越小 (mantissa 主导). 这是 FP8 香的原因.")
    ...
    print(f"   FP8 block=64: P50={np.percentile(err_fp8, 50)*100:.2f}%  P95={np.percentile(err_fp8, 95)*100:.2f}%  max={err_fp8.max()*100:.2f}%")
    print(f"   Q8  block=32: P50={np.percentile(err_q8, 50)*100:.2f}%  P95={np.percentile(err_q8, 95)*100:.2f}%  max={err_q8.max()*100:.2f}%")

    print("\n>>> Outlier 场景: 63 个 small (σ=0.1) + 1 个 100.0")
    np.random.seed(0)
    block = np.concatenate([np.random.randn(63) * 0.1, np.array([100.0])]).astype(np.float32)
    y_fp8 = fp8_quantize_block_inplace(block.copy(), block_size=64)
    y_q8 = q8_roundtrip(block.copy(), block_size=64)
    print(f"   原 outlier value: 100.0")
    print(f"   FP8 还原: {y_fp8[-1]:.2f}  Q8 还原: {y_q8[-1]:.2f}")
    print(f"   原 small[0]: {block[0]:.4f}")
    print(f"   FP8 还原: {y_fp8[0]:.4f}  Q8 还原: {y_q8[0]:.4f}")
    print(f"   ↑ Q8 scale 被 outlier 拉大, small values 损失大. FP8 用 exp 自适应, small 也保得住.")
    ...
    print(f"   1024 个 FP8:  {n_fp8_bytes} bytes  ({n_fp8_bytes*100/n_fp16_bytes:.0f}%)")
    print(f"   1024 个 Q8:   {n_q8_bytes} bytes  ({n_q8_bytes*100/n_fp16_bytes:.0f}%)")
