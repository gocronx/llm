...
def main() -> None:
    ...
    print("   wall-clock 用 stats × 时间常量估算, 不真 sleep (避免测试太慢)\n")
    ...
    print(
        f"{'draft acc':<10} {'lookahead':<10} {'target_calls':<14} {'draft_calls':<13} {'accept_rate':<13} {'wall_ms':<10} {'speedup':<10}"
    )
    print("-" * 90)
    ...
    for accuracy in [0.5, 0.7, 0.9]:
        ...
        for K in [2, 4, 8]:
            ...
            print(
                f"{accuracy:<10} {K:<10} {stats.target_calls:<14} {stats.draft_calls:<13} "
                f"{accept_rate:<13.2f} {wall:<10.1f} {speedup:<10.2f}×"
            )

    print("\n>>> 启用 margin filter (margin_threshold=1.5)")
    print(
        f"{'draft acc':<10} {'lookahead':<10} {'target_calls':<14} {'margin_filtered':<17} {'wall_ms':<10} {'speedup':<10}"
    )
    print("-" * 90)
    for accuracy in [0.5, 0.7]:
        ...
        for K in [4, 8]:
            _, stats = speculative_decode(
                prefix, n_steps, target, draft, lookahead=K, margin_threshold=1.5
            )
            wall = stats.target_calls * TARGET_MS + stats.draft_calls * DRAFT_MS
            speedup = naive_wall / wall
            print(
                f"{accuracy:<10} {K:<10} {stats.target_calls:<14} {stats.margin_filtered:<17} "
