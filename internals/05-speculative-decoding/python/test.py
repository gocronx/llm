...
def test_correctness_perfect_draft() -> bool:
    ...
    out_spec, stats = speculative_decode(
        [1, 2, 3], n_steps=20, target=target, draft=perfect_draft, lookahead=4
    )

    ok = out_naive == out_spec
    print(f"{'✓' if ok else '✗'} perfect draft → 输出与朴素一致")
    print(
        f"   stats: target_calls={stats.target_calls} (vs 朴素 {20}), accept={stats.accepted}, bonus={stats.bonus}"
    )
    return ok
...
def test_correctness_bad_draft() -> bool:
    ...
    out_spec, stats = speculative_decode(
        [1, 2], n_steps=15, target=target, draft=bad_draft, lookahead=4
    )
...
def test_correctness_mixed_draft() -> bool:
    ...
    out_spec, stats = speculative_decode(
        [10, 20, 30], n_steps=30, target=target, draft=mixed, lookahead=4
    )

    ok = out_naive == out_spec
    print(f"{'✓' if ok else '✗'} mixed draft → 输出与朴素一字不差 (lossless guarantee)")
    print(
        f"   stats: target_calls={stats.target_calls} (vs 朴素 30), accept={stats.accepted}, reject={stats.rejected}, bonus={stats.bonus}"
    )
    return ok
...
def test_speedup_proportional_to_accuracy() -> bool:
    ...
    for acc in [0.3, 0.5, 0.7, 0.9]:
        ...
        _, stats = speculative_decode(
            [1], n_steps=100, target=target, draft=draft, lookahead=4
        )
        speedup = 100 / stats.target_calls
        results.append((acc, stats.target_calls, speedup))
        print(
            f"   accuracy={acc}: target_calls={stats.target_calls}, 加速={speedup:.2f}×"
        )
    # 加速应单调增 (大概率)
...
def test_margin_filter_reduces_wasted_drafts() -> bool:
    ...
    _, no_filter = speculative_decode(
        [1], 50, target, draft, lookahead=8, margin_threshold=0.0
    )
    _, with_filter = speculative_decode(
        [1], 50, target, draft, lookahead=8, margin_threshold=1.5
    )

    print(
        f"   no_filter:  draft_calls={no_filter.draft_calls}, target_calls={no_filter.target_calls}"
    )
    print(
        f"   margin=1.5: draft_calls={with_filter.draft_calls}, target_calls={with_filter.target_calls}, margin_filtered={with_filter.margin_filtered}"
    )

    ok = (
        with_filter.margin_filtered > 0
        and with_filter.draft_calls < no_filter.draft_calls
    )
    print(
        f"{'✓' if ok else '✗'} margin filter reduces drafts (filtered {with_filter.margin_filtered})"
    )
    return ok
...
def test_bonus_token_on_full_accept() -> bool:
    ...
    _, stats = speculative_decode(
        [1], n_steps=20, target=target, draft=perfect, lookahead=4
    )
    # 全 accept → 每轮 K+1 个 token, K=4 时 4 轮就能搞 20 个
    # bonus 数 ≈ rounds (假设全 accept)
    print(
        f"   stats: rounds={stats.rounds}, accepted={stats.accepted}, bonus={stats.bonus}"
    )
    ok = stats.bonus > 0
