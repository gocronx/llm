...
def test_top_p_small_narrows() -> bool:
    ...
    rng = np.random.default_rng(0)
    out = Counter(
        sample(logits, temperature=1.0, top_k=0, top_p=0.5, min_p=0.0, rng=rng)
        for _ in range(2000)
    )
    ok = max(out.keys()) <= 2  # 应该集中在 id 0, 1, 最多 2
    print(
        f"{'✓' if ok else '✗'} top_p=0.5 narrows (used ids {sorted(out)}, ref probs {p.round(3)})"
    )
    return ok
...
def test_min_p_filters_long_tail() -> bool:
    ...
    rng = np.random.default_rng(0)
    out = Counter(
        sample(logits, temperature=1.0, top_k=0, top_p=1.0, min_p=0.5, rng=rng)
        for _ in range(2000)
    )
...
def test_pure_softmax_distribution() -> bool:
    ...
    cnt = Counter(
        sample(logits, temperature=1.0, top_k=0, top_p=1.0, min_p=0.0, rng=rng)
        for _ in range(N)
    )
    ...
    ok = l1 < 0.03
    print(
        f"{'✓' if ok else '✗'} pure softmax distribution (L1={l1:.4f}, ref={p_ref.round(3)}, emp={p_emp.round(3)})"
    )
    return ok
...
def test_top_p_keeps_at_least_one() -> bool:
    ...
    rng = np.random.default_rng(0)
    out = {
        sample(logits, temperature=1.0, top_k=0, top_p=1e-6, min_p=0.0, rng=rng)
        for _ in range(50)
    }
...
def test_high_temperature_uniform() -> bool:
    ...
    rng = np.random.default_rng(0)
    cnt = Counter(
        sample(logits, temperature=100.0, top_k=0, top_p=1.0, min_p=0.0, rng=rng)
        for _ in range(N)
    )
    p_emp = np.array([cnt.get(i, 0) / N for i in range(5)])
    # 跟 0.2 (1/5) 的距离应该不大
    ...
    ok = l1 < 0.05
    print(
        f"{'✓' if ok else '✗'} high temperature ~ uniform (L1={l1:.4f}, emp={p_emp.round(3)})"
    )
    return ok
