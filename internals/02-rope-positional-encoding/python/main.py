...
def main() -> None:
    ...
    print("   freq_scale = 4096 / 32768 = 0.125")
    print("   pos = 20000 (远超原训练范围)")
    for ext in [0.0, 0.5, 1.0]:
        out = apply_rope_yarn(
            x, pos=20000, n_ctx_orig=4096, freq_scale=0.125, ext_factor=ext
        )
        norm = np.linalg.norm(out)
        label = {0.0: "纯插值 PI", 0.5: "YaRN 中等混合", 1.0: "YaRN 完整"}[ext]
        print(
            f"   ext_factor={ext} ({label:<14}): out norm = {norm:.4f}, finite = {np.all(np.isfinite(out))}"
        )
    ...
    print(
        f"   freqs[16] (中频)  = {f_mid:.6f} → 转 1 周期需要 {2 * np.pi / f_mid:.2f} 个 token"
    )
    f_low = 10000 ** (-62 / 64)
    print(
        f"   freqs[31] (最低频) = {f_low:.6f} → 转 1 周期需要 {2 * np.pi / f_low:.2f} 个 token"
    )
