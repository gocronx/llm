...
def test_basic_generation() -> bool:
    ...
    out, stats = generate(
        model, prompt=[1, 2, 3], max_tokens=10, sample_fn=argmax_sampler
    )
    ok = (
        len(out) == 10
        and stats.generated_tokens == 10
        and stats.stop_reason == "max_tokens"
    )
    print(
        f"{'✓' if ok else '✗'} basic generation: {len(out)} tokens, stop={stats.stop_reason}, "
...
def test_eos_stop() -> bool:
    ...
    out, stats = generate(
        model, prompt=[0], max_tokens=20, sample_fn=sampler, eos_token=99
    )
    ok = stats.stop_reason == "eos" and out[-1] == 99 and len(out) == 5
    print(
        f"{'✓' if ok else '✗'} EOS stop: stopped at {len(out)} tokens, reason={stats.stop_reason}"
    )
    return ok
...
def test_ctx_size_stop() -> bool:
    ...
    out, stats = generate(
        model, prompt=[1, 2, 3], max_tokens=100, sample_fn=argmax_sampler
    )
    # ctx=10, prompt=3, 应能生成 ≤ 7 个 token 就触发 ctx_full
    ok = stats.stop_reason == "ctx_full" and len(out) < 10
    print(
        f"{'✓' if ok else '✗'} ctx_size stop: prompt=3+gen={len(out)}, ctx=10, reason={stats.stop_reason}"
    )
    return ok
...
def test_emit_callback_streaming() -> bool:
    ...
    out, stats = generate(
        ...
        prompt=[1],
        max_tokens=8,
        sample_fn=argmax_sampler,
        emit_fn=lambda t: emitted.append(t),
    )
    ok = emitted == out and len(emitted) == 8
    print(
        f"{'✓' if ok else '✗'} streaming emit: {len(emitted)} callbacks == {len(out)} tokens"
    )
    return ok
...
def test_kv_cache_length_tracks() -> bool:
    ...
    assert cache.len == 5
    print("✓ KV cache len tracks (prefill 4 + forward 1 → len=5)")
    return True
...
def test_empty_prompt_rejected() -> bool:
    ...
    try:
        ...
        print("✓ empty prompt rejected with ValueError")
        return True
...
def test_prompt_too_long_rejected() -> bool:
    ...
    try:
        ...
        print("✗ prompt > ctx_size not rejected")
        return False
    except ValueError:
        print("✓ prompt > ctx_size rejected with ValueError")
        return True
