# 21 · Context Governance

> 不熟 ReAct 范式? 先看 [09-simple-agent-demo](../09-simple-agent-demo) 的 "什么是 ReAct" 一节.

[09](../09-simple-agent-demo) 教了 ReAct 长啥样 —— 一个 71 行的 while 循环, 在玩具任务上跑 5 轮没问题. 但真把它塞到 production 里, 跑到 20+ 轮就开始死. 死的不是逻辑, 是 **context 形态**: API 返回 400, 或 LLM 因超 context window 拒绝.
