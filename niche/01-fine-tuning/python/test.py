...
def main() -> None:
    passed = sum(
        [
            t("good code passes 5/5", check(GOOD_CODE).score == 5),
            t("wrong framework caught", not check(WRONG_CODE).no_wrong_framework),
            t("syntax_ok on good", syntax_ok(GOOD_CODE)),
            t("syntax_ok rejects broken", not syntax_ok("def x( oops:")),
            # dataset
            t("build_samples returns 60+ samples", len(build_samples()) >= 60),
            t(
                "stratified_split covers all kinds",
                all(
                    any(s["kind"] == k for s in stratified_split(build_samples())[2])
            ...
            # summarize
            t(
                "summarize counts correctly",
                summarize([(check(GOOD_CODE), True), (check(WRONG_CODE), True)])[
                    "full_score"
                ]
                == 1,
            ),
