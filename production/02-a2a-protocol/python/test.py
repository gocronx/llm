...
def main() -> None:
    ...
    passed = sum(
        [
            t(
                "AgentCard auth optional",
                card.auth is None and card_with_auth.auth.scheme == "bearer",
            ),
            t("TaskRequest minimal", req.task_id == "t1" and req.input == {"x": 1}),
            t(
                "TaskResponse completed",
                resp_ok.state == "completed" and resp_ok.output == {"y": 2},
            ),
            t(
                "TaskResponse failed",
                resp_err.state == "failed" and resp_err.error == "oops",
            ),
            # extract_json_array
            t(
                "extract picks last valid array",
                extract_json_array('noise ["a","b"]') == ["a", "b"],
            ),
            t("extract empty if no array", extract_json_array("no array") == []),
            t(
                "extract handles nested arrays",
