...
def main() -> None:
    with tempfile.TemporaryDirectory() as d:
        ...
        passed = sum(
            [
                t(
                    "load_skills parses frontmatter",
                    len(skills) == 1
                    and skills[0].name == "test-skill"
                    and "翻译" in skills[0].triggers,
                ),
                t(
                    "load_skills_cached returns same on no change",
                    load_skills_cached(dpath) is load_skills_cached(dpath),
                ),
                t(
                    "route_keyword by trigger hit",
                    [s.name for s in route_keyword("帮我翻译一下", skills)]
                    == ["test-skill"],
                ),
                t(
                    "route_keyword no match → empty",
                    route_keyword("today's weather", skills) == [],
                ),
                t(
                    "compose no skills → no skill blocks",
                    "## Skill:" not in compose(skills, []).system,
                ),
                t(
                    "compose with skill embeds body",
                    "Body" in compose(skills, skills).system,
                ),
                t(
                    "_extract_array picks last array",
                    _extract_array('garbage ["a"]') == ["a"],
                ),
                t(
                    "_extract_array bad json → []",
