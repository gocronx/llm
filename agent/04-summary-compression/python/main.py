...
def make_realistic_history() -> list[dict]:
    ...
    fake_turns = [
        ...
        (
            "我看到 session 在 Redis 里, 现在需要看 user model",
            "read_file",
            '{"path":"models/user.py"}',
            "models/user.py: User class, password_hash 字段, 没有 jwt-related 字段",
        ),
        (
            "user model 没问题, 我需要加 JWT 工具函数",
            "write_file",
            '{"path":"auth/jwt.py","content":"..."}',
            "auth/jwt.py 已写入, 64 行, 含 sign_token 和 verify_token",
        ),
        (
            "接下来替换 middleware",
            "write_file",
            '{"path":"auth/middleware.py","content":"..."}',
        ...
        (
            "test_token_expiry 失败因为 exp 字段没设, 修复",
            "write_file",
            '{"path":"auth/jwt.py"}',
            "auth/jwt.py 更新, 加 exp = now + 24h",
        ),
        ("再跑一次测试", "run_tests", '{"path":"tests/auth"}', "12/12 passed ✓"),
        (
            "最后要更新文档",
            "write_file",
            '{"path":"docs/auth.md"}',
...
def main() -> None:
    ...
    compactor = LLMCompactor(
        ...
        CompactConfig(
            threshold_tokens=500, keep_recent_turns=2, target_summary_tokens=400
        ),
    )
