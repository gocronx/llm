...
class SubAgentResult:
    """Subagent 返回给主 agent 的"摘要包". 主 agent 不见 subagent 的完整对话."""
...
class SubAgent:
    ...
    def __init__(
        ...
        self.system_prompt = system_prompt or f"You are a {agent_type} subagent. Complete the task and report back."

    async def run(self, task: str, tools_allowed: list[str] | None = None) -> SubAgentResult:
        ...
        allowed = tools_allowed if tools_allowed is not None else list(self.tools.keys())
        ...
        for i in range(self.max_iterations):
            ...
            try:
                ...
                return SubAgentResult(
                    status="failed", summary="", n_iterations=iter_count,
                    elapsed_ms=(time.perf_counter() - t0) * 1000, error=str(e),
                )
            ...
            if not tool_calls:
                # LLM 给出最终 content, 结束
                return SubAgentResult(
                    status="completed", summary=last_content,
                    artifacts=artifacts, n_iterations=iter_count,
                    elapsed_ms=(time.perf_counter() - t0) * 1000,
                )

            messages.append({"role": "assistant", "tool_calls": tool_calls, "content": last_content})
        ...
        return SubAgentResult(
            status="partial",
            summary=last_content or f"(reached max_iterations={self.max_iterations} without final answer)",
            artifacts=artifacts, n_iterations=iter_count,
            elapsed_ms=(time.perf_counter() - t0) * 1000,
        )
...
class Orchestrator:
    ...
    async def delegate_parallel(
        ...
        tasks = [
            ...
            for (agent_type, task), allowed in zip(requests, tools_allowed_per)
        ]
