...
```mermaid
flowchart TD
    Q[新请求 prompt] --> E{精确缓存命中?}
    E -->|是| RE[返回缓存<br/>0 风险, 0 调用]
    E -->|否| S{语义相似度 ≥ 阈值?}

    S -->|是| RS[返回缓存<br/>有错答风险]
    S -->|否| API[调真实 LLM API<br/>服务端可能用前缀缓存]

    API --> WB[写回两层缓存]
    WB --> RES[返回新回答]

    classDef hit fill:#d4edda,stroke:#155724
    classDef warn fill:#fff3cd,stroke:#856404
    classDef miss fill:#f8d7da,stroke:#721c24
    class RE hit
    class RS warn
    class API,RES miss
```

风险递增、成本递减：精确缓存零误命中但命中率低，语义缓存命中率高但可能返回错答案，前缀缓存在服务端自动生效不需要应用层介入。
