"""Run the routing strategies against a small mixed query set, side by side."""
...
def _color_for_tier(tier: str) -> str:
    return {"cheap": Fore.GREEN, "mid": Fore.YELLOW, "premium": Fore.RED}[tier]


def _fmt(r: RouteResult) -> str:
    head = f"{_color_for_tier(r.chosen.tier)}{r.chosen.tier:<7}{Style.RESET_ALL} {r.elapsed_ms:>5}ms  ${r.cost:.5f}"
    if r.escalated_from:
...
def _print_registry() -> None:
    print(f"{Fore.CYAN}Registered models:{Style.RESET_ALL}")
    for m in REGISTRY:
    ...
    for name, fn in selected.items():
        ...
        for i, q in enumerate(queries, 1):
            try:
                ...
                print(f"  [{i}] {Fore.RED}ERROR{Style.RESET_ALL} {type(e).__name__}: {e}")
